...
struct RevenueView: View {
    ...
    private var entries: [RevenueEntry] {
        let all = store.revenues(businessId: activeBizId, year: selectedYear, month: selectedMonth)
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                if entries.isEmpty {
                    Spacer()
                    EmptyState(symbol: "dollarsign.circle", title: "No revenue yet.", caption: "Tap + to add.")
                    Spacer()
                } else {
                    List {
                            ...
                            RevenueRowView(entry: entry)
                                ...
                                .swipeActions(edge: .trailing, allowsFullSwipe: true) {
                                    Button(role: .destructive) {
                                        ...
                                        store.delete(revenue: entry, businessId: activeBizId, year: selectedYear, month: selectedMonth)
                                ...
                                .swipeActions(edge: .leading) {
                                    Button {
                                        ...
                                        copy.id = UUID()
                                        copy.createdAt = Date()
                                        store.upsert(revenue: copy, businessId: activeBizId, year: selectedYear, month: selectedMonth)
        ...
        .sheet(isPresented: $showingAdd) {
            AddRevenueView(existing: editingEntry, businessId: activeBizId) { entry in
                store.upsert(revenue: entry, businessId: activeBizId, year: selectedYear, month: selectedMonth)
                Haptics.success()
            }
            .environment(store)
        }
